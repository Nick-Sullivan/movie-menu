...
const manyRecipesMenu = {
  ...
  schedule: [
    ...
    },
    {
      ...
      recipe: {
        name: "Dessert",
        steps: [{ duration_secs: 120, note: "Plate" }],
      },
    },
  ],
};
...
async function mockApi(
  page: Page,
  overrides: Partial<typeof mockMenu> = {},
  startedAt?: number,
) {
  ...
  await page.route("http://localhost:3001/**", (route) => {
    ...
    if (method === "POST" && url.endsWith("/start")) {
      ...
      return route.fulfill({
        json: { ...menu, started_at: startedAt ?? body.start_at },
      });
    }
    ...
    if (method === "GET") return route.fulfill({ json: menu });
    if (method === "PUT") return route.fulfill({ json: menu });
...
async function goToEditView(
  page: Page,
  menuOverride: Partial<typeof mockMenu> = {},
  startedAt?: number,
) {
...
test("edit view - expand a dish shows steps that add up to when it appears", async ({
  page,
}) => {
  await goToEditView(page);
...
test("a recipe prep note shows before the steps, and in the chef timeline", async ({
  page,
}) => {
  const schedule = [
    {
...
test("edit view - dishes can be reordered", async ({ page }) => {
  ...
  await expect(
    page.locator('[aria-label="Move dish up"]').first(),
  ).toBeDisabled();
  await expect(
    page.locator('[aria-label="Move dish down"]').last(),
  ).toBeDisabled();
});
...
test("edit view - viewer screen settings", async ({ page }) => {
  ...
  await expect(
    page.locator('.viewer-setting input[type="checkbox"]'),
  ).toBeChecked();
});

test("edit view - screening time is always visible and defaults to now", async ({
  page,
}) => {
  await goToEditView(page);
  // fixed clock at 20:00, already on a 5-minute boundary
  await expect(page.locator("input.screening-input")).toHaveValue(
    "2026-07-06T20:00",
  );
  await expect(page.getByRole("button", { name: "Start film" })).toBeVisible();
});

test("edit view - warns when the screening time means prep is already missed", async ({
  page,
}) => {
    ...
    schedule: [
      {
        ...
        recipe: {
          name: "Roast",
          steps: [{ duration_secs: 1800, note: "Roast veg" }],
        },
      },
    ],
  });
...
test("running view - no back button; stopping the film returns to the edit page", async ({
  page,
}) => {
  await goToEditView(page, {}, FIXED_SECS - 300);
  await startFilm(page);
...
test("running view - future screening time counts down as negative", async ({
  page,
}) => {
  await goToEditView(page);
  ...
  await expect(page.locator(".film-clock-screening")).toHaveText(
    "starts 8:10 PM",
  );
...
test("running view - chef list highlights current step, viewer sees only allowed dishes", async ({
  page,
}) => {
  ...
  await goToEditView(
    page,
    {
      schedule: manyRecipesMenu.schedule,
  ...
  await expect(page.locator(".upcoming-note").first()).toHaveText(
    "Mystery dish",
  );
...
test("long chef list scrolls with the page — the list has no scrollbar of its own", async ({
  page,
}) => {
...
test("now serving shows the real dish name even when names are hidden", async ({
  page,
}) => {
  ...
  await goToEditView(
    ...
    {
      schedule: manyRecipesMenu.schedule,
      viewer: { upcoming_count: 1, show_dish_names: false },
    },
    FIXED_SECS - 1900,
  );
  await startFilm(page);
...
test("viewer view - no list when only the next dish is shown", async ({
  page,
}) => {
  await goToEditView(
    page,
...
test("joining a screening puts the code in the URL; back clears it", async ({
  page,
}) => {
  await goToEditView(page);
...
test("refresh during a screening returns to it via the code in the URL", async ({
  page,
}) => {
  await page.clock.setFixedTime(FIXED);
...
test("a plain code link is viewer-only; ?chef=true unlocks the chef screen", async ({
  page,
}) => {
  await page.clock.setFixedTime(FIXED);
  await mockApi(page, { started_at: FIXED_SECS - 300 });
...
test("a menu being edited survives a refresh without touching the server", async ({
  page,
}) => {
  await page.clock.setFixedTime(FIXED);
...
test("a code whose screening is not running reads as not found", async ({
  page,
}) => {
  ...
  await expect(page.locator(".error")).toHaveText(
    "No screening found with that code.",
  );
...
test("a bad code in the URL falls back to the home screen", async ({
  page,
}) => {
  await page.route("http://localhost:3001/**", (route) =>
...
test("a dish photo is downscaled to a local data URL and survives a refresh", async ({
  page,
}) => {
...
test("starting a screening uploads photos and sends the key, never the data URL", async ({
  page,
}) => {
  await goToEditView(page);
    ...
    route.fulfill({
      json: {
        uploads: [
          {
            key: "testkey0123",
            url: "http://localhost:3001/mock-s3/testkey0123",
          },
        ],
      },
    }),
  ...
  let putBody:
    | { schedule: Array<{ recipe: Record<string, unknown> }> }
    | undefined;
  await page.route("http://localhost:3001/menus/ABC12", (route) => {
    if (route.request().method() !== "PUT") return route.fallback();
    putBody = route.request().postDataJSON();
    return route.fulfill({
      json: { ...mockMenu, schedule: putBody!.schedule },
    });
  });
...
test("viewer sees the now-serving dish photo via the image endpoint", async ({
  page,
}) => {
  const schedule = [
    {
      ready_at_secs: 60,
      recipe: { name: "Popcorn", image_key: "photokey0001", steps: [] },
    },
    {
      ready_at_secs: 4000,
      recipe: { name: "Pasta", image_key: "photokey0002", steps: [] },
    },
  ];
  ...
  await expect(page.locator(".now-photo")).toHaveAttribute(
    "src",
    /\/images\/photokey0001$/,
  );
});
...
test("hidden dish names also hide upcoming photos", async ({ page }) => {
  const schedule = [
    ...
    {
      ready_at_secs: 5400,
      recipe: { name: "Dessert", image_key: "photokey0003", steps: [] },
    },
  ];
  await goToEditView(
    page,
